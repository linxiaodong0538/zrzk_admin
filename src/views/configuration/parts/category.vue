    ...
    <div class="app-container">
        ...
        >
            <template slot="fields">
                <x-fields :data="dialogForm['fields']"></x-fields>
            </template>
        </x-dialog>
...
import { DialogOptions, FormOptions } from "./category.data";
import { default as XFields } from "./fields.vue";

export default {
    components: { Treeselect, XTable, XForm, XTree, XDialog, XFields },
    data() {
        return {
            // 遮罩层
            loading: true,
            ...
            options: [
                { prop: "partsTypeName", label: "子类型" },
                { prop: "parentType", label: "主类型", formatter: this.mapping },
                {
                    prop: "fields",
                    label: "自定义属性",
                    formatter(row, column, index) {
                        return (row[column.property] || [])
                            .map(x => x.partsFieldNameCn)
                            .join(",");
                    }
                },
                    ...
                    },,
                    {
                        validator: async (
                            ...
                            if (
                                ...
                                const params = {
                                    id: this.dialogForm.partsTypeId,
                                    parentType: this.dialogForm.parentType,
                                    name: value
                                };
                                const { data } = await this.$partstype.validate(
                                    params
                                ); 
                                callback(
                                    data
                                        ? new Error("部件子类型名称已被使用")
                                        : undefined
                                );
                            }
                        },
    ...
    methods: {
        /** 查询用户列表 */
        search() {
            let start = this.formData.dateRange && this.formData.dateRange[0]
                ? moment(this.formData.dateRange[0]).format("YYYY-MM-DD")
                : undefined;
            let end = this.formData.dateRange && this.formData.dateRange[1]
            ...
            this.loading = true;
            this.$partstype.search(params).then(({ rows, total }) => {
                this.loading = false;
                this.models = rows;
        ...
        handleAdd(t) {
            this.dialogForm = {
                fields: [],
                parentType: this.clusterOptions && this.clusterOptions.length?this.clusterOptions[0].value: 'cover'
            };
        ...
        async handleUpdate(row) {
            const id = row && row.partsTypeId ? row.partsTypeId : this.ids[0];
            const { data } = await this.$partstype.get({id: id });
            this.dialogForm = Object.assign({}, data);
            this.dialogVisible = true;
        },
        /** 提交按钮 */
        submitForm: function() {
            if (this.dialogForm.partsTypeId != undefined) {
                this.$partstype.update(this.dialogForm).then(response => {
        ...
        /** 删除按钮操作 */
        handleDelete(row) {
            const ids = row && row.partsTypeId ? [row.partsTypeId] : this.ids;
            this.$confirm("是否确认删除数据项?", "警告", {
                ...
                .then(() => {
                    return this.$partstype.remove({ ids: ids });
                })
                .then(() => {
                    this.search();
        ...
        mapping(row, column, index) {
            ...
            if (property === "parentType") {
                result =
                    row[property] && this.cache.parentType[row[property]]
                        ? this.cache.parentType[row[property]]
                        : row[property] || "-";
            }
            return result;
        }
