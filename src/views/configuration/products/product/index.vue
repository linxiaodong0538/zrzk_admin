...
export default {
  ...
  data() {
    return {
      ...
      dialogRules: {
        extProductId: [{ required: true, message: "必填", trigger: "blur" }],
        productId: [{ required: true, message: "必填", trigger: "blur" }],
        productTypeId: [{ required: true, message: "必选", trigger: "change" }],
        productName: [
          { required: true, message: "必填", trigger: "blur" },
          {
            validator: async (rule, value, callback, source, options) => {
              ...
              if (value == "" || value == undefined || value == null) {
                ...
                const { data } = await this.$product.validate(params);
                callback(data ? new Error("产品名称已被使用") : undefined);
              }
            },
            trigger: "blur"
          }
          ...
          { required: true, message: "必填", trigger: "blur" },
          {
            validator: async (rule, value, callback, source, options) => {
              ...
              if (value == "" || value == undefined || value == null) {
                ...
                const { data } = await this.$product.validate(params);
                callback(data ? new Error("产品编号已被使用") : undefined);
              }
            },
            trigger: "blur"
          }
  ...
  created() {
    ...
    this.getDicts("protocol").then(({ data }) => {
      ...
      let node = this.dialogOptions.find(x => x.prop === "accessProtocol");
      if (node) {
    ...
    this.getDicts("protocol_platform").then(({ data }) => {
      ...
      let node = this.dialogOptions.find(x => x.prop === "protocolPlatform");
      if (node) {
