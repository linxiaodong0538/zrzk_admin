  ...
  <div class="xny-region">
    <div class="xny-search">
      ...
      <div class="xny-cover">
        <div class="xny-Manhole">
          <span class="coverList">类型名称:</span>
          <el-input placeholder size="small" class="Manhole" v-model="typeName" clearable />
      ...
      <div class="coverForm">
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              ...
              plain
              @click="handleAdd"
              v-hasPermi="['configuration:sensorType:add']"
            >新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="single"
              @click="handleUpdate"
              v-hasPermi="['configuration:sensorType:edit']"
            >修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete"
              v-hasPermi="['configuration:sensorType:remove']"
            >删除</el-button>
          </el-col>
          <!-- <el-col :span="1.5">
            <el-button
              ...
              plain
              @click="handleImport"
              v-hasPermi="['configuration:sensorType:import']"
            >导入</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              plain
              @click="handleExport"
              v-hasPermi="['configuration:sensorType:export']"
            >导出</el-button>
          </el-col>-->
        </el-row>
        <!-- v-loading="loading" -->
        <el-table :data="typeList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="类型名称" align="center" prop="sensorTypeName" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="类型名称: " prop="sensorTypeName">
          <el-input v-model="form.sensorTypeName" placeholder="类型名称" class="Manhole" clearable />
        </el-form-item>
...
import {
  ...
  getType,
  updateType
} from "@/api/product/sensorType.js";
          ...
          { required: true, message: "必填", trigger: "blur" },
          {
            validator: async (rule, value, callback, source, options) => {
              ...
              if (value == "" || value == undefined || value == null) {
                ...
                const params = {
                  id: this.form.sensorTypeId,
                  name: value
                };
                const { data } = await validate(params);
                callback(data ? new Error("类型名称已被使用") : undefined);
              }
            },
            trigger: "blur"
          }
  ...
  methods: {
    ...
    deviceSensorTypeAdd() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.sensorTypeId != undefined) {
            ...
            let sensorTypeName = this.form.sensorTypeName;
    ...
    handleUpdate(row) {
      this.reset();
      const id = row && row.sensorTypeId ? row.sensorTypeId : this.ids[0];
       getType(id).then(res => {
         console.log(res);
        this.form = res.data;
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const ids = row && row.sensorTypeId ? [row.sensorTypeId] : this.ids;
