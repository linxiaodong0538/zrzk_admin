...
import { DialogOptions, FormOptions } from "./monitor.data";
import BaiduMap from "vue-baidu-map/components/map/Map.vue";

export default {
  components: { BaiduMap, Treeselect, XTable, XForm, XTree, XDialog },
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  created() {
    ...
    this.$partstype.search({}).then(({ rows }) => {
      ...
      let node = this.dialogOptions.find(x => x.prop === "5");
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$refs.xtable && this.$refs.xtable.toggleAllSelection();
