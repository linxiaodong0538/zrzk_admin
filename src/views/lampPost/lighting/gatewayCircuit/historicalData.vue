<template>
  <div class="xny-region">
    ...
    <div class="xny-search">
      ...
      <div class="coverForm">
        ...
        <el-table :data="typeList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="更新时间" align="center" prop="alarmView" />
          <!-- fixed="right" -->
          <el-table-column label="回路开闭数据" min-width="140">
            <template slot-scope="scope">
              <span class="edit" @click="viewStatus">查看回路开闭状态</span>
            </template>
          </el-table-column>
          <el-table-column label="三相电数据">
            <template slot-scope="scope">
              <span class="edit" @click="viewThreePhasePower">查看三相电</span>
            </template>
          </el-table-column>
          <el-table-column label="回路电流数据" min-width="120">
            <template slot-scope="scope">
              <span class="edit" @click="electriccCrrent">查看回路电流</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- @pagination="getList" -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px">
      <viewCircuitStatus :id="management.id" :label="management.label" @cancel="cancel"></viewCircuitStatus>
    </el-dialog>
    <el-dialog :title="title" :visible.sync="ThreePhase" width="800px">
      <viewThreePhase :id="management.id" :label="management.label" @cancel1="cancel1"></viewThreePhase>
    </el-dialog>
    <el-dialog :title="title" :visible.sync="LoopCurrent" width="800px">
      <viewLoopCurrent :id="management.id" :label="management.label" @cancel2="cancel2"></viewLoopCurrent>
    </el-dialog>
  </div>
</template>
<script>
...
import viewCircuitStatus from "./viewCircuitStatus"; //查看回路开闭状态
import viewThreePhase from "./viewThreePhase"; //查看三相电
import viewLoopCurrent from "./viewLoopCurrent"; //查看回路电流
import { region, coverForm } from "@/api/system/cover/coverManagement";
import { treeselect } from "@/api/system/dept";
export default {
  components: { viewCircuitStatus, viewThreePhase,viewLoopCurrent },
  data() {
    return {
      // 区域/分组
      fla: 0,
