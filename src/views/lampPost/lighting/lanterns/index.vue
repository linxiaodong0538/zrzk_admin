<template>
    <div class="app-container">
        ...
        <el-dialog :title="title" :visible.sync="realTimeContro" width="800px">
            <viewDataAdd @cancel1="cancel1"></viewDataAdd>
        </el-dialog>
        <el-dialog :title="title" :visible.sync="writePolicy" width="800px">
            <electricEnergyAdd @cancel2="cancel2"></electricEnergyAdd>
        </el-dialog>
...
import { importTemplate } from "@/api/homePage/homePage";
import { monitoringExport } from "@/api/system/cover/coverManagement";
...
export default {
    ...
    created() {
        ...
        this.$lamp.getProducts({ productTypeId: 3 }).then(({ data }) => {
            this.products = data;
    ...
    methods: {
        //   实时控制
        contro() {
            this.realTimeContro = true;
            this.title = "实时控制";
        },
        cancel1(val) {
            this.realTimeContro = val;
            //   this.reset();
        },
        //写入策略
        policy() {
            ...
            this.$lamp.search(params).then(
                ({ rows, total }) => {
                    ...
                    this.models = rows.map(x => {
                        ...
                        if (
                            x.deviceFieldUploads &&
                            x.deviceFieldUploads.length
                        ) {
        ...
        view({ category, data }) {
            if (category === "history") {
                this.$router.push({
                   path: "/lightingHistory/",query:{deviceId:data.deviceId,deviceCode:data.deviceCode}
                });
            }
        ...
        /** 下载模板操作 */
        importTemplate() {
            let partsTypeId = 9999999;
            importTemplate(partsTypeId).then(response => {
                this.download(response.msg);
            });
        },
        // 文件上传中处理
